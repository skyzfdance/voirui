...
### 环境与依赖准备
- 最低环境要求
  - Node.js 版本范围：>=18 且 <=24
  - 包管理器：pnpm（版本在 package.json 中声明）
- 安装依赖
  - 在项目根目录执行安装命令，等待依赖下载完成
- 启动开发服务器
  - 运行开发脚本，Vite 将启动本地开发服务器并自动打开浏览器
- 生产构建
  - 执行构建脚本，生成 dist 目录中的静态资源
- 预览构建结果
  - 使用预览脚本在本地查看构建产物
...
## 故障排查指南
- 依赖安装失败
  - 确认 Node.js 版本满足 engines 要求（>=18 且 <=24）
